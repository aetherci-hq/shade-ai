...
export function LogLine({ ts, type, message }: LogLineProps) {
  ...
  return (
    <div className="flex gap-2 py-px animate-fade-in">
      <span className="text-c-muted shrink-0 text-[11px]">{formatTime(ts)}</span>
      <span className={`font-semibold shrink-0 min-w-[60px] uppercase text-[10px] tracking-[0.05em] ${color}`}>
        {shortType(type)}
      </span>
